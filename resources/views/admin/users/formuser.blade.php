<div class="box box-info padding-1">
    <div class="box-body">
        ...
        <div class="row">
            <div class="form-group  col-lg-6">
                {{ Form::label('password') }}
                {{ Form::text('password', $user->password, ['required', 'minlength' => '8', 'class' => 'form-control' . ($errors->has('password') ? ' is-invalid' : ''), 'placeholder' => 'Password ']) }}
                {!! $errors->first('password', '<div class="invalid-feedback">:message</div>') !!}
            </div>

            <div class="form-group  col-lg-6">
                {{ Form::label('password_confirmation', 'Password Confimación') }}
                {{ Form::text('password_confirmation', $user->password, ['required', 'minlength' => '8', 'class' => 'form-control' . ($errors->has('password') ? ' is-invalid' : ''), 'placeholder' => 'Password Confimación ']) }}
                {!! $errors->first('password', '<div class="invalid-feedback">:message</div>') !!}
            </div>
        </div>
        <div class="row">
            ...
            <div class="form-group col-lg-5">
                ...
                <select name='nacionalidad' id='nacionalidad' class='form-control'>
                    @foreach ($user->paises as $pais)
                        <option value="{{ $pais }}" @if (old('nacionalidad' == $pais)) selected @endif>
                            {{ $pais }}</option>
                    @endforeach
                </select>
        ...
        <div class="row">
            ...
            <div class="form-group col-lg-3">
                ...
                <label class="mr-2">
                    {!! Form::radio('estado_wsp', 'Validado', $user->estado_wsp == 'Validado', ['checked']) !!}
                    Validado
                </label>
                @error('estado_wsp')
            ...
            <div class="form-group col-lg-3">
                ...
                <label class="mr-2">
                    {!! Form::radio('verificado', 'Si', $user->verificado == 'Si', ['checked']) !!}
                    Si
                </label>
                @error('verificado')
                    <span class="text-danger">{{ $message }}</span>
